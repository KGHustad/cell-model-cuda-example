...
// Init state values
__global__ void init_state_values(double *d_states, const unsigned int num_cells, const unsigned int padded_num_cells)
{
...
__global__ void step_FE(double *__restrict d_states, const double t, const double dt,
                        const double *__restrict d_parameters, const unsigned int num_cells, const unsigned int padded_num_cells)
{
    const int thread_ind = blockIdx.x * blockDim.x + threadIdx.x;
...
int choose_threadblock_size(int device_number, enum solver_scheme kernel)
{
    ...
    for (int tbs = 32; tbs <= deviceProp.maxThreadsPerBlock; tbs *= 2) {
        ...
        switch (kernel) {
        case SCHEME_FE:
            checkCuda(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks,
                step_FE,
                tbs, 0
            ));
            break;
...
int main()
{
    int num_cells = (int) 1E7;
    int num_timesteps = 10;
    double dt = 1E-3;
    enum solver_scheme scheme = SCHEME_FE;
    //scheme = SCHEME_GRL1;
    int device_id = 0;
    ...
    init_state_values<<<threadblocks, threadblock_size, 0, stream>>>(d_states, num_cells, padded_num_cells);
    ...
    // synchonize the stream to ensure that the init kernel has completer before starting the timer and entering the main loop
    ...
    for (int i = 0; i < num_timesteps; i++) {
        ...
        {
        case SCHEME_FE:
            step_FE<<<threadblocks, threadblock_size, 0, stream>>>(d_states, t, dt, d_parameters, num_cells, padded_num_cells);
            break;

        case SCHEME_GRL1:
            step_GRL1<<<threadblocks, threadblock_size, 0, stream>>>(d_states, t, dt, d_parameters, num_cells, padded_num_cells);
            break;
        }
